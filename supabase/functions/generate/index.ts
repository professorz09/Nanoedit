...
function makeVertex(): any {
  ...
  if (key) {
    // Gemini 3.x image models (gemini-3-pro-image-preview / gemini-3.1-flash-
    // image-preview) are ONLY served from the `global` location on Vertex — an
    // API-key client that defaults to a regional endpoint gets a 404
    // "Publisher model not found". Pin it explicitly.
    return new GoogleGenAI({
      vertexai: true,
      apiKey: key,
      location: Deno.env.get('VERTEX_LOCATION') || 'global',
...
Deno.serve(async (req) => {
  ...
  const attempts: Array<{ name: string; run: () => Promise<GenResult> }> = [
    ...
    // (Imagen removed — deprecated, shuts down 2026-08-17; the Gemini-3 Flash
    //  model already covers text-to-image and honours aspect ratio.)
  ...
  // Log provider failures even when a LATER provider succeeded — lets us see why
  // e.g. Vertex fell through to OpenRouter without failing the whole request.
  if (errors.length) console.error('provider_errors', usedProvider ? `(recovered via ${usedProvider}) ` : '', errors.join(' | '));

  if (!gen || !gen.images.length) {
    // Total failure across all providers → refund. A failed generation is free.
    // supabase-js rpc() returns a thenable builder, NOT a real Promise — it has
    // no .catch(), so calling one throws an UNCAUGHT TypeError that crashes the
    // handler into a CORS-less 500 (surfaces in the browser as "Could not reach
    // the server") AND skips the refund. Wrap in try/catch instead.
    try { await admin.rpc('refund_credit', { p_user: user.id }); } catch (_) { /* best-effort refund */ }
